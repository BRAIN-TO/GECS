...
from .. models.pdcm import *
from .. models.hemodynamic import *

__all__ = ['spm_dx', 'spm_int', 'spm_int_IT']
...
def spm_int_IT(P, M, U, nargout=1):
    ...
    # get expansion point
    # --------------------------------------------------------------------------
    x = spm.vec(M['x'])  # matlab: [1; spm_vec(M.x)]
#    print(x.shape, spm.unvec(x, M['x']).shape)
...
#        M['g'] = g
    ...
    for i in range(size(t)):
        ...
        # ----------------------------------------------------------------------
        q = spm.unvec(x, M['x'])
        fx, dfdx   = M['f'](q,U['u'][t[i],:],P,M, nargout=2)
        ...
        x = x + np.expand_dims(dt[i]*fx + 0.5*dt[i]**2*(dense(dfdx)@fx), 1)
