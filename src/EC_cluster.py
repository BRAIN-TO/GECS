...
def main(args):
    ...
    df_dict_new = torch.load(args.input_filename)
    ...
    for validation in range(0,total_subjects):
      ...
      df_dict_validation = df_dict_new.iloc[validation:validation + 1].drop('labels', axis=1)
      df_dict_all_std = df_dict_new.drop('labels', axis=1).drop(validation)
      ...
      new_labels = list(df_dict_new.drop(validation).labels)
      target = new_labels
